async function updateLicense() {
    ...
    const confirmUpdate = confirm(
      `Are you sure you want to update the selected license`
    );
  
    if (checkboxes.length === 0) {
      alert("You have not selected an license");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/update-license-status`, {
        method: "PUT",
        headers: {
    ...
    // ——————— Bersihkan seleksi ———————
    localStorage.removeItem("selectedLicenseIds");
